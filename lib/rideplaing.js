...
// ============================================================
// TIME CONVERSION HELPERS - For DateTime storage
// ============================================================

/**
 * Converts a time string (like "6:00 PM") to a Date object
 * using the provided rideDate as the base date.
 * This stores the time as a proper DateTime in the database.
 */
function timeStringToDate(timeStr, baseDate) {
  ...
  // Parse time string like "6:00 PM" or "8:00 PM"
  try {
    ...
    if (match) {
      ...
      // Create a Date object using the rideDate as base
...
/**
 * Extracts time string from various formats for logging/debugging
 */
...
async function syncPendingRidesForWeek(weekStartDate) {
  ...
  for (const driverSchedules of byDriverTrip.values()) {
    ...
    for (const dayKey of DAY_KEYS) {
      ...
      // ✅ FIX: Convert time strings to Date objects for DateTime fields
      const pickupDate = timeStringToDate(primary.pickupTime, rideDate);
      const officeArrivalDate = timeStringToDate(
        primary.officeArrivalTime,
        rideDate,
      );
      const dropDate = timeStringToDate(primary.dropTime, rideDate);

      // For logging/debugging
      const pickupStr = extractTimeString(primary.pickupTime);
      const officeArrivalStr = extractTimeString(primary.officeArrivalTime);
      const dropStr = extractTimeString(primary.dropTime);
      ...
      rideWritePlans.push({
        ...
        migrated,
        dayEligible,
        // Store formatted strings for logging
        pickupStr,
        officeArrivalStr,
        dropStr,
      });
