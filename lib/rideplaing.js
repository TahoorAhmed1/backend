...
// ---------------------------------------------------------------------------
// NEW: Normalize the many shapes callers use for scope.
// Supports: driverId, driverIds, tripId, tripIds, routeId, routeIds,
//           vehicleId, vehicleIds, skipNotifications (boolean)
// ---------------------------------------------------------------------------
function normalizeScope(scope = {}) {
  const toArray = (v) =>
    v == null ? [] : Array.isArray(v) ? v.filter((x) => x != null) : [v];
  const merge = (single, multi) =>
    [...new Set([...toArray(single), ...toArray(multi)])];

  return {
    driverIds: merge(scope.driverId, scope.driverIds),
    tripIds: merge(scope.tripId, scope.tripIds),
    routeIds: merge(scope.routeId, scope.routeIds),
    vehicleIds: merge(scope.vehicleId, scope.vehicleIds),
    skipNotifications: scope.skipNotifications === true,
  };
}
...
//
// scope = {
//   driverId | driverIds, tripId | tripIds, routeId | routeIds,
//   skipNotifications?: boolean   // default false
// }
//
// Returns:
//   {
//     results,                  // per-ride sync results (existing shape)
//     affectedEmployeeIds: [],  // unique employee ids touched this run
//     affectedDriverIds: [],    // unique driver ids touched this run
//   }
//
// When scope.skipNotifications is true, this function performs NO
// notification at all — the caller is responsible for notifying.
// ---------------------------------------------------------------------------
async function syncPendingRidesForWeek(weekStartDate, scope = {}) {
  const {
    driverIds: scopeDriverIds,
    tripIds: scopeTripIds,
    routeIds: scopeRouteIds,
    vehicleIds: scopeVehicleIds,
    skipNotifications,
  ...
  const isScoped =
    ...
    scopeVehicleIds.length;

  console.log("[rideSync] weekStartDate", weekStartDate, "scope", scope);
  ...
  //
  // NEW: if the caller passed skipNotifications: true, we skip this and
  // return the affected IDs so the caller can send its own, more specific
  // notification — this prevents duplicate notifications.
  console.log('skipNotifications', skipNotifications)
  if (!skipNotifications) {
    ...
    console.log(
      `[rideSync] skipNotifications=true — suppressing rideSync notification. ` +
        `affectedEmployeeIds=${affectedEmployeeIds.size}, affectedDriverIds=${affectedDriverIds.size}`,
    );
  }
