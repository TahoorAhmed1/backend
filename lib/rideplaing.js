...
async function syncPendingRidesForWeek(weekStartDate) {
  ...
  // Track ALL affected employees (deduplicated by employeeId)
  // This includes both drivers AND passengers
  const allAffectedEmployeeIds = new Set();
  ...
  for (const driverSchedules of byDriverTrip.values()) {
    ...
    for (const dayKey of DAY_KEYS) {
      ...
      // Track ALL unique employees (drivers + passengers)
      // Driver
      if (driverId) {
        allAffectedEmployeeIds.add(driverId);
      }
      // Passengers
      for (const s of dayEligible) {
        if (s.employeeId) {
          allAffectedEmployeeIds.add(s.employeeId);
        }
      }
    }
  }
  ...
  // ============================================================
  // SEND ONE NOTIFICATION PER EMPLOYEE (DRIVERS + PASSENGERS)
  // ============================================================

  // const affectedEmployeeIds = Array.from(allAffectedEmployeeIds);

  // if (affectedEmployeeIds.length > 0) {
  //   console.log(
  //     `[ridePlanning] Sending ${affectedEmployeeIds.length} notification(s) (one per employee, not per ride)`,
  //   );

  //   // Send ONE notification per employee (fire and forget)
  //   Promise.all(
  //     affectedEmployeeIds.map(async (employeeId) => {
  //       try {
  //         const result = await notifyEmployeeScheduleUpdatedBestEffort(
  //           employeeId,
  //           weekStartDate,
  //           {
  //             message: "Your weekly ride schedule has been synced",
  //           },
  //         );
  //         return { employeeId, success: true, result };
  //       } catch (error) {
  //         console.error(
  //           `[ridePlanning] Failed to notify employee ${employeeId}:`,
  //           error,
  //         );
  ...
  //       const succeeded = notificationResults.filter((r) => r.success).length;
  //       const failed = notificationResults.filter((r) => !r.success).length;
  //       console.log(
  //         `[ridePlanning] Notifications: ${succeeded} employee(s) notified, ${failed} failed`,
  //       );
  //     })
  //     .catch((error) => {
  //       console.error(`[ridePlanning] Error sending notifications:`, error);
  //     });
  // } else {
  //   console.log(`[ridePlanning] No employees to notify`);
  // }
